...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def async_update(api_category: str) -> dict[str, Any]:
        ...
        try:
            if api_category == CATEGORY_USER_REPORT_INDIVIDUAL:
                data = await client.user_data.async_get_nearest_by_coordinates(
                    latitude, longitude
                )
            elif api_category == CATEGORY_USER_REPORT_TOTALS:
                data = await client.user_data.async_get_totals()
            else:
                data = await client.cdc_data.async_get_nearest_by_coordinates(
                    latitude, longitude
                )
